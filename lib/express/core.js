      ...
      .createServer(function(req, response){
        ...
        if (request.header('content-type') &&
          ...
          var stream,
              ...
              maxBodyLength = dsl.set('max upload size')
  ...
  route: function(request){ 
    ...
    request.trigger('request', function(err) {
      try {
        ...
        if (typeof (body = (new exports.Router(request)).route()) === 'string')
...
dsl.configure(function(){
  plugin.use(require('express/plugins').View)
  plugin.use(require('express/plugins').Cache)
  plugin.use(require('express/plugins').Redirect)
  plugin.use(require('express/plugins').BodyDecoder)
})
...
dsl.configure('production', function(){
  ...
  dsl.enable('cache view partials')
