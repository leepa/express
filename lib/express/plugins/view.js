...
exports.View = Plugin.extend({
  extend: {
    ...
    init: function() {
      ...
      Request.include({
        ...
        render: function(view, options) {
          ...
          options.context = options.context || helpers
    ...
    /**
     * Add each key / value in _methods_ as
     * view helpers, with an optional _context_
     * which becomes "this" when executed.
     *
     * @param  {hash} methods
     * @param  {mixed} context
     * @api public
    ...
    helpers: function (methods, context) {
      $(methods).each(function(fn, name){
        helpers[name] = context ?
          function(){ return fn.apply(context, arguments) } :
            fn
      })  
    }
